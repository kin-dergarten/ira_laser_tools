...
LaserscanMerger::LaserscanMerger() : Node("laserscan_multi_merger")
{
    ...
    RCLCPP_INFO(this->get_logger(), "\033[1;32mINIT LASERSCANMERGER\033[0m");
    RCLCPP_INFO(this->get_logger(), "\033[1;32mLASERSCAN_TOPICS: {}\033[0m", laserscan_topics);
...
void LaserscanMerger::createSubscribers()
{
    try {
        ...
        // Resize vectors to match the number of input topics
        scan_subscribers.resize(input_topics.size());
        clouds_modified.resize(input_topics.size(), false);
        ...
        for (std::vector<int>::size_type i = 0; i < input_topics.size(); ++i)
        {
            ...
            RCLCPP_INFO(this->get_logger(), "\033[32msubscribed topics:\033[0m");
    ...
    RCLCPP_INFO(this->get_logger(), "\033[32mCREATED_SUPSCRIPTIONS\033[0m");
}
